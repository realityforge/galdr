...
def load_diagnostic_messages
  JSON.load(IO.read("#{WORKSPACE_DIR}/core/src/test/java/galdr/diagnostic_messages.json"))
end
...
task 'error_codes:print_new_error_code' do
  puts "Galdr-#{sprintf('%04d', load_diagnostic_messages.collect{|m|m['code']}.sort.last.to_i + 1)}"
end
...
task 'error_codes:next' do
  ...
  1.upto(max_value).each do |v|
    unless keys.delete(v)
      puts "Galdr-#{'%04d' % v} (previously used)"
      found = true
      break
    end
  end
  puts "Galdr-#{sprintf('%04d',max_value + 1)} (new)" unless found
end
