...
public final class Galdr_DeltaParameterEntityProcessorSubSystem implements PostConstructFn, OnActivateFn, OnDeactivateFn, ProcessorFn {
  @Nonnull
  ...
  private static final class EnhancedSubSystem extends DeltaParameterEntityProcessorSubSystem {
    @Nonnull
    private final Galdr_DeltaParameterEntityProcessorSubSystem $galdr$_outer;
    ...
    private EnhancedSubSystem(@Nonnull final Galdr_DeltaParameterEntityProcessorSubSystem outer) {
      $galdr$_outer = Objects.requireNonNull( outer );
    }

    private void $galdr$_postConstruct() {
      $galdraoi$_process = $galdr$_outer.$galdr$_getWorld().createAreaOfInterest( Collections.singleton( DeltaParameterEntityProcessorSubSystem.MyComponent.class ) );
    }
    ...
    private void $galdr$_process(final int delta) {
      ...
      while ( -1 != ( $galdrs$_process_entityId = $galdrs$_process.nextEntity() ) ) {
        process( delta, $galdrs$_process_entityId );
      }
    }

    @Nonnull
    private String $galdr$_getName() {
      return "DeltaParameterEntityProcessorSubSystem";
    }
