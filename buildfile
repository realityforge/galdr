...
define 'galdr' do
  ...
  ipr.add_testng_configuration('core',
                               :module => 'core',
                               :jvm_args => '-ea -Dbraincheck.environment=development -Dgaldr.environment=development -Dgaldr.check_diagnostic_messages=false -Dgaldr.output_fixture_data=false -Dgaldr.diagnostic_messages_file=core/src/test/java/galdr/diagnostic_messages.json')
